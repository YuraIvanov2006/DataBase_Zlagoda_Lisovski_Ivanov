...
import { formatDateInput, money, parseFullName } from '../../utils/formatters.js';
...
export function EmployeesPage() {
  ...
  const sortedRows = useMemo(() => {
    const r = sortRows(rows, sortState.key, sortState.dir, sortState.type);
    return r;
  }, [rows, sortState]);
  ...
  const validate = (f, isEdit) => {
    ...
    if (f.emplZipCode?.length > 9) err.emplZipCode = 'До 9 символів';
  ...
  const openEdit = async (row) => {
    ...
    try {
      ...
      const { surname, firstName, patronymic } = parseFullName(data.fullName);
      setForm({
        emplSurname: surname,
        emplName: firstName,
        emplPatronymic: patronymic,
        emplRole: (data.emplRole || 'CASHIER').toUpperCase().includes('MANAG')
          ? 'MANAGER'
          : 'CASHIER',
        salary: data.salary ?? '',
        dateOfBirth: '',
        dateOfStart: formatDateInput(data.dateOfStart),
        emplPhoneNumber: data.emplPhoneNumber || '',
        emplCity: '',
        emplStreet: '',
        emplZipCode: '',
      });
