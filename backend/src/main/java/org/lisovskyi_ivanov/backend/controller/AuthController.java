...
public class AuthController {
    ...
    public record LoginRequest(
        ...
        @NotBlank(message = "Пароль обов'язковий") @Size(
            min = 6
        ) String password
    ) {}
    ...
    public ResponseEntity<?> login(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                Map.of("message", "Невірний логін або пароль")
            );
        }
    }
    ...
    public ResponseEntity<?> me(Authentication authentication) {
        if (authentication == null || !authentication.isAuthenticated()) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                Map.of("message", "Не авторизовано")
            );
        }

        Account account = accountDetailsService.findByLogin(
        ...
        String fullName =
            ...
            (employee.getEmplPatronymic() != null
    ...
    public ResponseEntity<Void> register(
        ...
        Account account = Account.builder()
            .employee(
                Employee.builder().idEmployee(request.idEmployee()).build()
            )
            .login(request.login())
